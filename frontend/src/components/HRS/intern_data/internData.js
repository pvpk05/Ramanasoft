...
const InternsTable = ({ currentInterns, handleDelete, handleToggleBlock, handleEditClick }) => {
  ...
  const columnDefs = [
    { headerName: 'Intern ID', field: 'candidateID', sortable: true, filter: true },
    {
      ...
      cellRenderer: params => (
        <Link
          to={`/HR_dash/student/${params.data.candidateID}`}
...
const EditModal = ({ show, onHide, intern, onSave }) => {
  ...
  const validationSchema = Yup.object({
    ...
    address: Yup.string()
      ...
      .max(100, 'Address must be less than or equal to 30 characters'),
    ...
    batchNo: Yup.string().required("Batch No is required"),
      ...
      dateAccepted: Yup.date()
        ...
        .test('start-end-different', 'Start Date and End Date should be different', function(value) {
          const { endDate } = this.parent; // Access the endDate field from the parent form
          return value !== endDate;  // Ensure the startDate is not the same as the endDate
        }),
      ...
      endDate: Yup.date()
        ...
        .test('start-end-different', 'Start Date and End Date should be different', function(value) {
          const { dateAccepted } = this.parent; // Access the dateAccepted field from the parent form
          return value !== dateAccepted;  // Ensure the endDate is not the same as the startDate
        })    
