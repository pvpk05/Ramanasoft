...
const LMSDashboard = () => {
  ...
  const handleDeleteTopic = async (courseName, topicName) => {
    try {
      const res = await apiService.delete(`/api/delete_topic/${courseName}/${topicName}`);
      console.log(res);
      fetchCourses();
    } catch (error) {
      console.error(`Error deleting course:`, error);
  ...
  const handleDeleteSubTopic = async (courseName, topicName, subTopicName) => {
    try {
      await apiService.delete(`/api/delete_subtopic/${courseName}/${topicName}/${subTopicName}`);
      fetchCourses();
    } catch (error) {
      console.error(`Error deleting course:`, error);
    }
                ...
                {Object.entries(courseData.topics).map(([topicName, topicData]) => (
                  <React.Fragment key={topicName}>
                    <ListItem style={{ paddingLeft: 50 }}>
                      ...
                      <IconButton onClick={() => handleDeleteTopic(courseName, topicName)} style={{ fontSize: "12px", alignItems: "center", gap: "2px" }}>
                        <DeleteIcon /> Topic
                      </IconButton>
                        ...
                        {Object.entries(topicData.subTopics).map(([subTopicName, subTopicData]) => (
                          <React.Fragment key={subTopicName}>
                            <ListItem style={{ paddingLeft: 100 }}>
                              ...
                              <IconButton onClick={() => handleDeleteSubTopic(courseName, topicName, subTopicName)} style={{ fontSize: "12px", alignItems: "center", gap: "2px" }}>
                                <DeleteIcon /> SubTopic
                              </IconButton>
