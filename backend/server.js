...
app.post('/api/sendEmail', async (req, res) => {
  ...
  if (!email || !mailOptions || !mailOptions.subject) {
    return res.status(400).json({ status: 400, message: 'Email and mail options are required, including subject and text' });
  }

  const result = await sendEmail(email, mailOptions);
...
app.put('/api/intern_data/:candidateID', async (req, res) => {
  ...
  const {
    ...
    modeOfInternship, profile_img, dateAccepted, endDate
  } = req.body;

  try {
    // Query to check if the record exists in the certificates table for the given internID (candidateID)
    const checkCertificateQuery = `
      SELECT * FROM certificates WHERE internID = ?;
    `;

    const certificateExists = await query(checkCertificateQuery, [candidateID]);

    // If the certificate exists, update the certificates table first
    if (certificateExists.length > 0) {
      const certificateUpdateQuery = `
      ...
      await query(certificateUpdateQuery, [
        fullName, domain, dateAccepted, endDate, candidateID
      ]);
    }
    ...
    await query(updateQuery, [
      fullName, email, mobileNo, altMobileNo, domain,
      belongedToVasaviFoundation, address, batchNo,
      modeOfInternship, profile_img, dateAccepted, endDate, candidateID
    ]);
...
app.post("/api/accept-guests", async (req, res) => {
  ...
  try {
    ...
    // Send emails to all registered interns
    try {
      const emailPromises = acceptedGuests.map(guest => {
        const mailOptions = {
          name: `${guest.fullName}`,
          subject: 'Welcome to RamanaSoft',
          htmlBody: `
                <div style="font-family:'New Century Schoolbook, TeX Gyre Schola, serif';">
                  <div style="background-color: #0a1b3d; padding: 20px; text-align: center;">
                    <h1 style="color: #fff; margin: 0;">Welcome To <br /> RamanaSoft Consultancy Services</h1>
                  </div>
                  <div style="padding: 20px; font-size:20px;">
                    <p style="font-size:23px;">Here are your account details,</p>
                    <p><strong>Candidate ID:</strong> ${guest.guestID}</p>
                  ...
                  <div style="padding-bottom:10px;">
                    Login to <a href='https://ramanasoft.com/login' style="font-size:23px">RamanaSoft</a> to proceed furthur.
                  </div>
                    <p>Best Regards,<br>Team RamanaSoft.</p>
                  </div>
                  <div style="background-color: #f5f5f5; padding: 10px; text-align: center; font-size: 12px;">
...
app.get('/api/show_certificate/:id', (req, res) => {
  const internID = req.params.id; // Extracting internID from params
  const sql = 'SELECT * FROM certificates WHERE internID = ?';

  query(sql, [internID], (err, rows) => {
    console.log(rows);
    if (err) {
      console.error('Error fetching certificates:', err);
      return res.status(500).json({ error: 'Internal Server Error' });
...
app.post('/api/save_certificate_data', async (req, res) => {
  const { internID, studentName, domain, position, certificationId, startDate, endDate } = req.body;

  const sql = 'INSERT INTO certificates (internID, studentName, domain, position, certificationId, startDate, endDate) VALUES (?, ?, ?, ?, ?, ?, ?)';

  query(sql, [internID, studentName, domain, position, certificationId, startDate, endDate], (err, result) => {
    if (err) {
      console.error(err);
...
// Delete Topic
app.delete('/api/delete_topic/:courseName/:topicName', async (req, res) => {
  const { courseName, topicName } = req.params;
  console.log(courseName, topicName);
  try {
    const sql = 'DELETE FROM Course WHERE CourseName = ? AND Topic = ?';
...
// Delete Topic
app.delete('/api/delete_subtopic/:courseName/:topicName/:subTopicName', async (req, res) => {
  const { courseName, topicName, subTopicName } = req.params;
  console.log(courseName, topicName, subTopicName);
  try {
    const sql = 'DELETE FROM Course WHERE CourseName = ? AND Topic = ? AND SubTopic = ?';
    await query(sql, [courseName, topicName, subTopicName], (err) => {
      if (err) {
        console.error('Error deleting Sub Topic:', err);
        return res.status(500).json({ error: 'Failed to delete Sub topic' });
      }
      return res.status(200).json({ message: 'Sub Topic deleted successfully' });
    });
  } catch (error) {
    console.error('Server Error:', error);
    return res.status(500).json({ error: 'Server Error' });
...
app.get('/api/dashboard-achievements-status/:internID', async (req, res) => {
  const internID = req.params.internID;
  try {
    const result = await query(`SELECT endDate FROM intern_data WHERE candidateID = ?`, [internID]);
    if (!result || result.length === 0) {
      return res.status(404).json({ error: 'Intern not found' });
    }
    const endDate = new Date(result[0].endDate); // Assuming the date is returned in a valid format
    const currentDate = new Date();
    const totalAchievements = endDate > currentDate ? 1 : 2;
    const Achievements = {
  ...
  try {
    ...
    const sqlQuery = `
      SELECT 
        ...
        SUM(CASE WHEN jobs.status = 'profiles-sent' THEN 1 ELSE 0 END) AS sent_profiles,
        SUM(CASE WHEN jobs.status IN ('drive-done', 'offer-received') THEN 1 ELSE 0 END) AS drives_done,
        COUNT(DISTINCT jobs.jobId) - SUM(CASE WHEN jobs.status IN ('drive-done', 'offer-received') THEN 1 ELSE 0 END) AS drives_pending
      FROM 
        hr_data
      LEFT JOIN jobs ON jobs.postedBy = hr_data.HRid
      WHERE 
        jobs.last_updated BETWEEN ? AND ?
      GROUP BY 
        hr_data.fullName, hr_data.HRid
      ORDER BY 
        hr_data.fullName;
    ...
    console.error("Error fetching HR statistics:", err);
    res.status(500).json({ message: "Server Error" });
  }
});
...
app.get('/api/domain-stats', async (req, res) => {
  console.log("Domain Stats API called");
  const domainOptions = [
    "Android Developer", "Python Full Stack", "Java Full Stack", "Mern Full Stack",
    "Testing Tools", "Scrum Master", "Business Analyst", "Dot Net",
    "Data Science", "Cyber Security", "Cloud Data Engineer",
    "DevOps & Cloud Computing",
    "Project Management & Agile", "SalesForce",
  ...
  // Calculate startDate and endDate based on year and month
  if (!year) {
    startDate = '1900-01-01';  // Default to far past if no year is given
    endDate = new Date().toISOString().split('T')[0];  // Current date
  } else if (month) {
    startDate = `${year}-${month.padStart(2, '0')}-01`;  // First day of the given month
    endDate = new Date(year, month, 0).toISOString().split('T')[0];  // Last day of the given month
  } else {
    startDate = `${year}-01-01`;  // First day of the given year
    endDate = `${year}-12-31`;  // Last day of the given year
  }
  ...
  try {
    ...
    const sqlQuery = `
      ...
      LEFT JOIN 
          applied_students a ON a.jobID = j.jobId
      WHERE
          j.last_updated BETWEEN ? AND ?  -- Filter jobs based on the calculated date range
      GROUP BY 
          td.domain
      ORDER BY 
          total_jobs_posted DESC, td.domain;
    ...
    // Execute the query with the calculated startDate and endDate
    const result = await query(sqlQuery, [startDate, endDate]);

    // Drop the temporary table
    await query('DROP TEMPORARY TABLE IF EXISTS temp_domains');

    // Return the filtered domain stats
    res.status(200).json(result);
  } catch (err) {
...
// API endpoint to save domains and batches
app.post("/api/save-domain", async (req, res) => {
  const { domain, batches } = req.body;
  const values = batches.map((batch) => [domain, batch]);
  const sql = "INSERT INTO domains_batches (domain, batch) VALUES ?";
...
app.delete("/api/delete-domain/:id", async (req, res) => {
  const { id } = req.params;
  try {
    await query("DELETE FROM domains_batches WHERE id = ?", [id]);
...
app.get("/api/domain_data", async (req, res) => {
  try {
    const rows = await query('SELECT * FROM domains_batches');
    res.status(200).json(rows);
  } catch (err) {
    console.error("Database query error: ", err);
    res.status(500).json({ message: "Server error" });
...
app.get('/api/blocked_data', async (req, res) => {
  try {
    const interns = await query('SELECT * FROM intern_data WHERE blockProfile = "1"');
    const guests = await query('SELECT * FROM guest_data WHERE blockProfile = "1"');
...
app.post("/api/check-mobile-availability", async (req, res) => {
  const { mobileno } = req.body;
  console.log(req.body);
  try {
    const mobileExists = await query("select * from intern_data where mobileNo = ?", [mobileno]);
    console.log(mobileExists);
    if (mobileExists.length > 0) {
      return res.status(200).json({ exists: true, message: "Mobile number already exists." });
    }
    else {
    ...
    console.error("Error fetching mobile exists :", err);
    res.status(500).json({ message: "" });
  }
});


app.post("/api/check-email-availability", async (req, res) => {
  const { email } = req.body;
  console.log(req.body);
  try {
    const emailExists = await query("select * from intern_data where email = ?", [email]);
    ...
    console.error("Error fetching email exists :", err);
    res.status(500).json({ message: "" });
  }
});
